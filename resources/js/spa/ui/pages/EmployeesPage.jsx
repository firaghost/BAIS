...
export function EmployeesPage() {
    ...
    const openEdit = (emp) => {
        ...
        setEditPayload({
            id: emp?.id ?? null,
            branch_id: emp?.branch_id != null ? String(emp.branch_id) : String(emp.branch?.id ?? ''),
    ...
    const submitCreate = async () => {
        ...
        const payload = {
            branch_id: createPayload.branch_id ? Number(createPayload.branch_id) : null,
    ...
    const submitEdit = async () => {
        ...
        const payload = {
            branch_id: editPayload.branch_id ? Number(editPayload.branch_id) : null,
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (!res.ok) {
                setBranchesState({ status: 'error', data: [], error: res.error });
                return;
            }

            const list = Array.isArray(res.data?.data) ? res.data.data : [];
            setBranchesState({ status: 'success', data: list, error: null });
        })();
    ...
    const activeFilterChips = useMemo(() => {
        ...
        if (branchId) {
            const b = branchesById.get(branchId);
            chips.push({ key: 'branch', label: `Branch: ${b?.name ?? branchId}` });
        }
        ...
        <div className="space-y-6 h-full flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Manage personnel records across all enterprise branches.</p>
                </div>
                        ...
                        <div className="flex-1 overflow-auto">
                            ...
                            <div className="p-4 border-b border-slate-200 space-y-3">
                                ...
                                <div className="flex items-center gap-3 text-sm">
                                    ...
                                    <span className="text-slate-600">{selectedEmployee.branch_name || selectedEmployee.branch?.name || '—'}</span>
                                </div>
                                <div className="flex items-center gap-3 text-sm">
                        ...
                        <div className="p-4 overflow-auto flex-1 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select className="mt-1 w-full rounded border-slate-200" value={createPayload.branch_id} onChange={(e) => setCreatePayload((p) => ({ ...p, branch_id: e.target.value }))}>
                                        <option value="">—</option>
                                        {branches.map((b) => (
                                            <option key={b.id} value={String(b.id)}>
                                                {b.name}
                    ...
                    <div className="w-full max-w-2xl max-h-[calc(100vh-2rem)] rounded-lg bg-white shadow-xl flex flex-col overflow-hidden">
                        ...
                        <div className="p-4 overflow-auto flex-1 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700">Branch</label>
                                    <select className="mt-1 w-full rounded border-slate-200" value={editPayload.branch_id} onChange={(e) => setEditPayload((p) => ({ ...p, branch_id: e.target.value }))}>
                                        <option value="">—</option>
                                        {branches.map((b) => (
                                            <option key={b.id} value={String(b.id)}>
                                                {b.name}
