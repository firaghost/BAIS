...
void main() async {
  ...
  stdout.writeln('Width: ${image.width}, Height: ${image.height}');
  ...
  for (int y = 0; y < image.height; y++) {
    for (int x = 0; x < image.width; x++) {
      ...
      if (pixel.a > 10) {
        // arbitrary small non-transparent threshold
        if (x < minX) minX = x;
        if (x > maxX) maxX = x;
        if (y < minY) minY = y;
        if (y > maxY) maxY = y;
  ...
  stdout.writeln('Non-transparent bounds: ($minX, $minY) to ($maxX, $maxY)');
  ...
  stdout.writeln(
    'Center pixel: R=${centerPixel.r}, G=${centerPixel.g}, B=${centerPixel.b}, A=${centerPixel.a}',
  );

  // What color is the edge?
  final pxIn10 = image.getPixel(cx, minY + 10);
  stdout.writeln(
    'Edge inside bound y+10: R=${pxIn10.r}, G=${pxIn10.g}, B=${pxIn10.b}, A=${pxIn10.a}',
  );
  ...
  if (cnt > 0) {
    stdout.writeln(
      'Avg color of non-transparent: R=${rSum ~/ cnt}, G=${gSum ~/ cnt}, B=${bSum ~/ cnt}',
    );
  }
}
